...
export default function CreateShipmentStep2() {
    ...
    const [weight, setWeight] = useState(draft.weightKg);
    const [loadType, setLoadType] = useState(draft.loadType);
    const [dimensions, setDimensions] = useState(draft.dimensions);
    const [notes, setNotes] = useState(draft.notes);

    const handleNext = () => {
        if (!weight.trim() || !Number.isFinite(parseFloat(weight))) return;
        set({
