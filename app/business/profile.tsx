...
export default function Profile() {
    ...
    const load = useCallback(async () => {
        try {
            const [me, shipments] = await Promise.all([
                api.me(),
                api.listMyShipments(),
            ]);
            setProfile(me);
            const items = shipments.items || [];
            const completed = items.filter((s: any) => s.status === 'delivered' || s.status === 'completed').length;
            const active = items.filter((s: any) => !['delivered', 'completed', 'cancelled'].includes(s.status)).length;
            setStats({ total: items.length, completed, active });
        } catch (e) {
            console.warn('business profile load failed', e);
        }
    }, []);
                ...
                >
                    ...
                    >
                        ...
                        <View style={styles.profileSection}>
                            ...
                            <Text style={styles.userName}>{profile?.companyName || user?.name || 'Business'}</Text>
                        ...
                        <View style={styles.section}>
                            ...
                            <View style={styles.menuContainer}>
                                ...
                                <MenuItem
                                    ...
                                    color="#10B981"
                                    showDivider={false}
                                    onPress={() => router.push('/business/payment-methods' as any)}
                                />
                            </View>
                        </View>
                        ...
                        <View style={styles.section}>
                            ...
                            <View style={styles.menuContainer}>
                                <MenuItem icon="help-circle-outline" label="Help Center" color="#F59E0B" onPress={() => router.push('/business/help' as any)} />
                                <MenuItem icon="settings-outline" label="Settings" onPress={() => router.push('/business/settings' as any)} />
                                <MenuItem icon="document-text-outline" label="Terms & Conditions" showDivider={false} onPress={() => router.push('/business/terms' as any)} />
                            </View>
                        </View>
