...
type ShipmentRow = {
    id: string; pickupAddress: string; dropAddress: string;
    truckType: string; createdAt: string; price: number; status: string;
};
...
export default function BusinessHome() {
    ...
    const load = useCallback(async () => {
        try {
            const list = await api.listMyShipments();
            const items = (list.items || []) as ShipmentRow[];
            const active = items.filter(s => s.status === 'in-transit').length;
            const pending = items.filter(s => s.status === 'active').length;
            const completed = items.filter(s => s.status === 'completed').length;
            const w = await api.wallet();
            setStats({ active, completed, pending, balance: w.balance });
            setActiveShipments(
                items.filter(s => s.status === 'in-transit' || s.status === 'active').slice(0, 2)
            );
        } catch (e) { console.warn('biz home load failed', e); }
    }, []);
        ...
        <View style={styles.container}>
            ...
            <SafeAreaView style={styles.safeArea}>
                ...
                >
                    ...
                    {/* Live Tracking — only render when there's a real in-transit shipment */}
                    {(() => {
                        const live = activeShipments.find(s => s.status === 'in-transit');
                        if (!live) return null;
                        return (
                            <Animated.View
                            ...
                            >
                                <View style={styles.section}>
                                    ...
                                    <TouchableOpacity
                                        activeOpacity={0.9}
                                        onPress={() => handleViewShipment(live.id)}
                                    >
                                        <LinearGradient
                                                        ...
                                                        <View style={styles.liveHeaderLeft}>
                                                            ...
                                                            <View>
                                                                <Text style={styles.liveTitle} numberOfLines={1}>
                                                                    {live.pickupAddress} → {live.dropAddress}
                                                                </Text>
                                                                <Text style={styles.liveSubtitle}>{live.truckType}</Text>
                                                            </View>
                                                        </View>
                                                    ...
                                                    <View style={styles.liveFooter}>
                                                        <View style={styles.liveFooterItem}>
                                                            ...
                                                            <View style={styles.liveFooterTextContainer}>
                                                                <Text style={styles.liveFooterLabel}>Price</Text>
                                                                <Text style={styles.liveFooterValue}>
                                                                    ₹{Math.round(live.price).toLocaleString()}
                                                                </Text>
                                                            </View>
                                                        </View>
                                                        <View style={styles.liveFooterItem}>
                                                            ...
                                                            <View style={styles.liveFooterTextContainer}>
                                                                <Text style={styles.liveFooterLabel}>Booked</Text>
                                                                <Text style={styles.liveFooterValue}>
                                                                    {new Date(live.createdAt).toLocaleDateString('en-IN', { month: 'short', day: 'numeric' })}
                                                                </Text>
                                                            </View>
                                                        </View>
                                                    </View>
                            ...
                            <View style={styles.statCard}>
                                ...
                                >
                                    <BlurView intensity={30} tint="light" style={styles.statBlur}>
                                        ...
                                        <Text style={styles.statLabel}>Awaiting driver</Text>
                                    </BlurView>
                                </LinearGradient>
                            </View>
                        </View>
                        ...
                        <View style={styles.section}>
                            ...
                            {activeShipments.length === 0 ? (
                                <Text style={{ color: textSecondary, textAlign: 'center', marginVertical: 24 }}>
                                    No active shipments. Tap "New Shipment" to book.
                                </Text>
                            ) : activeShipments.map(s => (
                                <TouchableOpacity
                                    key={s.id}
                                    activeOpacity={0.9}
                                    onPress={() => handleViewShipment(s.id)}
                                        ...
                                        <BlurView intensity={35} tint="light" style={styles.shipmentBlur}>
                                            ...
                                            <View style={styles.shipmentDetails}>
                                                <View style={styles.shipmentDetailItem}>
                                                    ...
                                                    <Text style={styles.shipmentDetailText}>{s.truckType}</Text>
                                                </View>
                                                <View style={styles.shipmentDetailItem}>
                                                    <LinearGradient
                                                        colors={['rgba(245, 158, 11, 0.15)', 'rgba(251, 191, 36, 0.1)']}
                                            ...
                                            <View style={styles.shipmentFooter}>
                                                <Text style={styles.shipmentPrice}>₹{Math.round(s.price).toLocaleString()}</Text>
