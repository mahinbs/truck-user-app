...
import { useShipmentDraft } from '../../contexts/CreateShipmentContext';
import {
    Alert,
    Dimensions,
    SafeAreaView,
...
import { getSyncedPickupLocation } from '../../utils/geo';
...
export default function CreateShipmentStep1() {
    ...
    const [pickup, setPickup] = useState(draft.pickupAddress);
    const [dropoff, setDropoff] = useState(draft.dropoffAddress);
    const [saving, setSaving] = useState(false);

    useEffect(() => {
        if (!draft.pickupAddress && !draft.dropoffAddress) reset();
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);

    const handleNext = async () => {
        if (!pickup.trim() || !dropoff.trim() || saving) return;
        setSaving(true);
        try {
            const loc = await getSyncedPickupLocation();
            set({
            ...
            Alert.alert(
                'Location required',
                'Allow location access so we can find drivers near your pickup.',
            );
        } finally {
            setSaving(false);
        }
        ...
        <View style={styles.container}>
            ...
            <SafeAreaView style={styles.safeArea}>
                ...
                <View style={styles.footer}>
                    <Button
                        title={saving ? 'Getting location…' : 'Next'}
                        onPress={handleNext}
